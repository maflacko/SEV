...
pyplot.xlabel('Étudiants')
pyplot.ylabel('Note')
pyplot.title("Notes obtenues à l'unité {}".format(Code_Unite))
...
tableau_notes = [['Étudiant', 'Note', 'Résultat']]
for etudiant in notes:
    note = notes[etudiant]
    # Condition pour déterminer le résultat
    if note >= 10:  # Supposons que 10 est la note de validation
        resultat = "Vous validez la matière"
    else:
        resultat = "Vous êtes au rattrapage"
    tableau_notes.append([etudiant, note, resultat])
